...
/*BUTTON_PIN (PB.12 on the board)*/
#define BUTTON_PIN          MCU_GPIO_PORTB(12)
/*HALL_EFFECT (PB.14 on the board)*/
...
static void my_hall_ev_close_cb(struct os_event *);


/*Decalartion of event queue*/
static struct os_eventq my_timer_button_eventq;
static struct os_eventq my_timer_hall_eventq_close;
...
static void 
blinker_LED (int led, float tps, int cycle)
{
    int i;
    for (i=0; i<cycle; ++i)
    {
        set_LED(led);
        os_time_delay(tps*OS_TICKS_PER_SEC);
        clear_LED(led);
        os_time_delay(tps*OS_TICKS_PER_SEC);
    }
}
 ...
 * at pin LED_2.
 */
static void my_hall_ev_close_cb(struct os_event *ev)
{
    assert(ev!=NULL);
    set_LED(g_led_pin2);
}
...
static void 
my_hall_irq_close(void *arg)
{
    os_eventq_put(&my_timer_hall_eventq_close, &gpio_ev1);
}
...
my_timer_hall_task(void *arg)
{
    while(1)
    {
        os_eventq_run(&my_timer_hall_eventq_close);
    }
}
...
init_tasks(void)
{
    ...
    os_callout_init(&my_hall_callout, &my_timer_hall_eventq_close, 
    ...
    hal_gpio_irq_init(HALL_EFFECT, my_hall_irq_close, NULL,
